...
*/
    ...
    public function remove_from_team($student_id, $course_id) {
        ...
        $teams = $student->teams()->where('team.course_id', $course_id);
    ...
    public function add_team(Request $request) {
    	...
    	$team->save();
        $url = route('course.manage.teams', $request->course_id);
        return redirect($url);
    ...
    public function remove_team(Request $request) {
    	...
    	$team->delete();
    	return redirect(route('course.manage.teams', $course_id));
    }
...
//    	return response()->json($skill);
    ...
    public function leave($user_id) {
        ...
        return redirect()->route('students.manage', $course->id)->withFlashSuccess($user->name . " has been removed from this course.");
...
//    	$skills = $course->skills()->orderBy('name')->get();
        ...
        return view('frontend.manage.course.details', ['course' => $course, 'zones' => $tzlist, 'course_id' => $course_id])
            ->withUser(access()->user());
